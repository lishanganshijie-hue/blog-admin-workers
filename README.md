...
## 20260511新增功能
优化【文章管理】页面加载速度
- 新增优先极速获取posts-index.json的文章目录
- 需要在cloudflare的worker【设置】中启动【构建缓存】，从缓存40-50秒提速至1-3秒
- 其次保底递归获取所有子目录中的md文章（较慢但稳）

## 20260513新增功能
![](https://views.ccwu.cc/img/2026/05/13/1778666266367.png)
新增【草稿箱】，并在文章管理页面将原【文章列表】更改为【已发布文章】：
- 新建文章时：你写完内容，直接点页面右上角左边的 “暂存草稿”按钮，它会自动给文件名加 draft- 前缀，存入 GitHub，并出现在你的草稿箱里。
- 编辑已有文章（已发布）时：如果你点 “暂存草稿”，它会把这篇文章从正式列表里“撤回”，重命名为 draft-xxx.md 并移入草稿箱。
- 编辑草稿时：点 “暂存草稿”：继续保持草稿状态保存。
- 点 “保存发布”：系统检测到没勾选草稿（因为 savePost 默认逻辑是看勾选框），它会自动去掉 draft- 前缀，把文章“转正”发布。
